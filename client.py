...
class Client:
    ...
    async def transfer_data(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
        ...
        asyncio.run_coroutine_threadsafe(common.transfer_data_with_compress(reader, remote_writer), self.loop)
        asyncio.run_coroutine_threadsafe(common.transfer_data_with_decompress(remote_reader, writer), self.loop)
